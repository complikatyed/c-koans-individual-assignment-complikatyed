...
namespace DotNetKoans.CSharp
{
    public class AboutArrays : Koan
    {
        ...
        public void CreatingArrays()
        {
            ...
            Assert.Equal(typeof(object[]), empty_array.GetType());  // CREATE AN ARRAY BY CALLING A NEW object[]

            //Note that you have to explicitly check for subclasses
            Assert.True(typeof(Array).IsAssignableFrom(empty_array.GetType()));

            Assert.Equal(0, empty_array.Length); // AN EMPTY ARRAY'S LENGTH IS ZERO
        }
        ...
        public void ArrayLiterals()
        {
            ...
            //Are arrays 0-based or 1-based?   ANSWER: ZERO BASED
            ...
            List<int> dynamicArray = new List<int>();  // IF YOU'RE GOING TO NEED TO ADD TO THE ARRAY, CONSIDER USING A LIST INSTEAD 
            dynamicArray.Add(42);
        ...
        public void AccessingArrayElements()
        {
            ...
            Assert.Equal("peanut", array[0]);  // FIRST ITEM IN THE ARRAY IS ACCESSED WITH array[0]
            Assert.Equal("jelly", array[3]);  // FOURTH ITEM IN ARRAY IS ACCESSED WITH array[3]
        ...
        public void SlicingArrays()
        {
            ...
            // THESE TWO ARE EXPECTING A NEW ARRAY MADE FROM THE THINGS THAT WERE REMOVED, NOT THE THINGS THAT ARE LEFT IN THE ARRAY
        }
    }
}
